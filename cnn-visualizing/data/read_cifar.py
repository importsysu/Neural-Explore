...
cifar_path = '~/Document/ResearchMeterials/cifar'
...
class Cifar100Reader(object):
	"""Class Cifar-100 Reader reads data from pickled cifar-100 data
	...
	def __init__(self, role):
		self.role=role
		if role == 'train':
			rawdata = unpickle(os.path.join(cifar_path, 'cifar-100-python/train'))
		elif role == 'test':
			rawdata = unpickle(os.path.join(cifar_path, 'cifar-100-python/test'))
		else:
	...
	def get_meta_info(self):
		return unpickle(os.path.join(cifar_path, 'cifar-100-python/meta'))
...
class Cifar10Reader(object):
	...
	def __init__(self, role):
		...
		if role == 'train':
			...
			rawdata = unpickle(os.path.join(cifar_path, 'cifar-10-batches-py/test_batch'))
		else:
			raise ValueException('Role is invalid for Cifar-100: {}'.format(role))
