...
class AlexNetConfig(object):
    ...
    def __init__(self, num_classes, keep_prob, skip_layer):
...
class AlexNet(Model):
    ...
    def __init__(self, x, config:AlexNetConfig, weights_path='DEFAULT'):
        ...
        #elf.x = tf.placeholder(tf.float32, [None] + config.image_shape)
        #self.y = tf.placeholder(tf.float32, [None, config.num_classes])
        self.x = x
        self.config = config
        self.NUM_CLASSES = config.num_classes
        self.KEEP_PROB = config.keep_prob
        ...
        if weights_path == 'DEFAULT':
            self.WEIGHTS_PATH = '/Users/zuo/code/bvlc_alexnet.npy'
        else:
    ...
    def create(self):
        ...
        dropout6 = _dropout(fc6, self.KEEP_PROB)

        # 7th Layer: FC (w ReLu) -> Dropout
        fc7 = fc(dropout6, 4096, 4096, name='fc7')
        dropout7 = _dropout(fc7, self.KEEP_PROB)
...
def swith_dropout(x, cond, keep_prob):
    return tf.cond(cond, lambda: tf.nn.dropout(x, keep_prob), lambda: x)
